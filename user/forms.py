...
class RegForm(forms.Form):
    username = forms.CharField(
        ...
        max_length=20,
        min_length=6,
        widget=forms.TextInput(attrs={'class': "form-control",'placeholder': '请输入6-20位的用户名'}),
    )
    ...
    password = forms.CharField(
        label='密码',
        min_length=6,
        widget=forms.PasswordInput(attrs={'class': "form-control", 'placeholder': '请输入最少6位密码'})
    )
...
class ChangePasswordForm(forms.Form):
    ...
    def clean_old_password(self):
        ...
        if not self.user.check_password(old_password):
            raise forms.ValidationError('原来的密码错误')
        return old_password
