...
def get_blog_list_common_date(request, blogs_all_list):
    paginator = Paginator(blogs_all_list, settings.EACH_PAGE_BLOGS_NUMBER)  # 每EACH_PAGE_BLOGS_NUMBER篇进行分页
    page_num = request.GET.get('page', 1)  # 获取url的页面参数（GET请求）
    page_of_blogs = paginator.get_page(page_num)
    current_page_number = page_of_blogs.number  # 获取当前页码
    page_range = list(range(max(current_page_number - 2, 1), current_page_number)) + \
                 list(range(current_page_number, min(current_page_number + 2, paginator.num_pages) + 1))
    # 加上省略页码标记
    if page_range[0] - 1 >= 2:
        page_range.insert(0, '...')
    ...
    for blog_date in blog_dates:
        blogs_count = Blog.objects.filter(created_time__year=blog_date.year,
                                          created_time__month=blog_date.month).count()
        blog_dates_dict[blog_date] = blogs_count
...
def blog_details(request, blog_pk):
    ...
    comment = Comment.objects.filter(content_type=blog_content_type, object_id=blog_pk, parent=None)'''  # 使用tag实现

    context = {}
    context['blog'] = blog
    context['previous_blog'] = Blog.objects.filter(created_time__gt=blog.created_time).last()
    context['next_blog'] = Blog.objects.filter(created_time__lt=blog.created_time).first()
    '''context['comments'] = comment.order_by('-comment_time')'''  # 使用tag实现

    '''context['comment_form'] = CommentForm(initial={'content_type': blog_content_type.model,
                                                   'object_id': blog_pk, 'reply_comment_id': 0})'''  # 使用tag实现
    response = render(request, 'blog_detail.html', context)   # 响应
    response.set_cookie(read_cookies_key, 'true')   # 阅读cookie标记
    return response
